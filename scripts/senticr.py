#Changes_made
"""
    1. Decreased the value of repeat
    2. Decreased the value of train-test-split
    3. Changed the training dataset
"""
...
# keywords
 ...
 'if', 'do', 'include', 'each', 'than', 'finally', 'class', 'double', 'float', 'numpy.int64','else','instanceof',
...
class SentiCR:
    ...
    def create_model_from_training_data(self):
        ...
        smote_model = SMOTE(random_state=None,  k_neighbors=15, 
                     n_jobs=1)
    ...
    def read_data_from_oracle(self):
        ...
        workbook = pd.read_csv("StackOverflow.csv")
        sheet = workbook
        oracle_data=[]
        print("Reading data from oracle..")

        for index, row in sheet.iterrows():
            comments = SentimentData(row['comments'], row['ratings'])
            oracle_data.append(comments)
        # for cell_num in range(0, sheet.shape[0]):
        #     # comments=SentimentData(sheet(cell_num, 0).value,sheet(cell_num, 1).value)
        #     comments = SentimentData(sheet.iloc[cell_num][0],sheet.iloc[cell_num][1])
        #     oracle_data.append(comments)
...
def ten_fold_cross_validation(dataset, ALGO):
    ...
    # classifier_model=SentiCR(algo=ALGO,training_data= dataset) 
    # print(dataset.shape)
    # # Cynthia
    # test_data = pd.read_csv('SO_Workflow_Data.csv')
    # # test_data = test_data.iloc[1500:]
    # # test_comments=[comments.text for comments in dataset[test]]
    # # test_ratings=[comments.rating for comments in dataset[test]]
    # test_comments = test_data[["Body"]]
    # test_ratings = test_data[["RatingsSentiCR"]]
    # print(test_comments)
    # print(test_ratings)
    ...
    for train, test in kf.split(dataset):
        ...
        precision = precision_score(test_ratings, pred, pos_label=-1, average='weighted')
        recall = recall_score(test_ratings, pred, pos_label=-1, average='weighted')
        f1score = f1_score(test_ratings, pred, pos_label=-1, average='weighted')
        accuracy = accuracy_score(test_ratings, pred)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str,
                        help='Classification algorithm', default="MLPC")


    parser.add_argument('--repeat', type=numpy.int64,
 ...
 # workbook = open_workbook("oracle.xlsx")
    ...
    # oracle_data = pd.DataFrame(oracle_data)
    # print(oracle_data.head(10))

    oracle_data=numpy.array(oracle_data)
